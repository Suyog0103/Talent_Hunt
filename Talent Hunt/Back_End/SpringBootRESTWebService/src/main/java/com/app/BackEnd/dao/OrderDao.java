...
public interface OrderDao extends JpaRepository<Order1, Integer> {
	...
	@Modifying
    @Query(value = "insert into order1 (order_id,booked_date,booked_time,order_price,venue,artist_email, email) VALUES (default, :booked_date,:booked_time,:order_price,:venue,:artist_emailid,:user_email)", nativeQuery = true)
    @Transactional
...
//	@Query("select b from Artist a ,Order1 b where artist_emailid=artist_artist_emailid and artist_artist_emailid=?1")  //not working
	@Query(value="select * from Order1 where artist_email=?", nativeQuery = true)
...
//	@Query(value="select * from Order1 where email=?", nativeQuery = true)
	@Query(value="select c.artist_name, c.artist_genre, c.artist_email, o.booked_date, o.order_price, o.email, o.order_id, o.booked_time, o.order_date, o.venue FROM artist c INNER JOIN Order1 o ON c.artist_email = o.artist_email where o.email=?", nativeQuery = true)
