...
booked(booked_date,artistfees,artist_email){
  ...
  handleBooking = (singerId) => {
    const updatedSingers = this.state.singerarr.map((singer) =>
      singer.id === singerId ? { ...singer, booked: true } : singer
    );
    this.setState({ singers: updatedSingers });
  };
