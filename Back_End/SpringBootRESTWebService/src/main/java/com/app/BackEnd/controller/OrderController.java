...
//		if (list.isEmpty()) {
	...
	@PutMapping("/confirmbooking/{artistemail}")
	public void confirmBooking(@PathVariable String artistemail) {
		System.out.println("in confirm booking controller");
		 orderService.confirmbooking(artistemail);
	}
	
	@GetMapping("/bookedartistbyuemail/{email}")
	public List<Order1> getbookedartistByUserEmail(@PathVariable String email) {
		System.out.println("in booked artist controller which is after payment");
		return orderService.getbookedartistByUserEmail(email);
	}
